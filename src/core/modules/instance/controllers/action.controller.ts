...
export class ActionController {
  ...
  listenForStateChangeHooks = () => {
    this.InstanceService.StateController.StateObserver.subscribe(
      (previous, updated) => {
        if (updated)
          if ("$onEnter" in this.ActionsObject[updated])
            this.ActionsObject[updated]?.$onEnter?.(previous, updated);
        if (previous)
          if ("$onLeave" in this.ActionsObject[previous])
            this.ActionsObject[previous]?.$onLeave?.(previous, updated);
      },
    );
  };
  ...
  ThrowWrongStateErr = (actionName: string, expectedState: string) => {
    const currentState =
      this.InstanceService.StateController.StateObserver.get();
    throw new Error(
      `Action ${actionName} cannot be executed in ${currentState}. Expected state: ${expectedState}`,
    );
  };

  decorateWithStateGuard = (
    method: AnyFunction,
    actionName: string,
    ...
    return (...args: any[]) => {
      ...
      const currentState =
        this.InstanceService.StateController.StateObserver.get();
      if (!isHook && currentState !== expectedState)
        this.ThrowWrongStateErr(actionName, expectedState);
  ...
  createObjectOfActions = () => {
    ...
    for (const state in schema) {
      for (const action in schema[state]) {
        ...
        schema[state][action] = this.decorateWithStateGuard(
          method,
          action,
          state,
        );
